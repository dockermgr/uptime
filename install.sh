...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202309090237-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.pro
# @@License          :  LICENSE.md
# @@ReadME           :  install.sh --help
# @@Copyright        :  Copyright: (c) 2023 Jason Hempstead, Casjays Developments
# @@Created          :  Saturday, Sep 09, 2023 02:37 EDT
# @@File             :  install.sh
# @@Description      :  Container installer script for uptime
...
REPO="${DOCKERMGRREPO:-https://github.com/$SCRIPTS_PREFIX}/uptime"
APPVERSION="$(__appversion "$REPO/raw/$REPO_BRANCH/version.txt")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Defaults variables
APPNAME="uptime"
export INSTDIR="$HOME/.local/share/CasjaysDev/$SCRIPTS_PREFIX/uptime"
export DOCKERMGR_CONFIG_DIR="${DOCKERMGR_CONFIG_DIR:-$HOME/.config/myscripts/$SCRIPTS_PREFIX}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set the mountpoint directory
export APPDIR="$HOME/.local/share/srv/docker/uptime"
...
HUB_IMAGE_URL="casjaysdevdocker/uptime"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set container hostname and domain - Default: [uptime.$SET_HOST_FULL_NAME] [$SET_HOST_FULL_DOMAIN]
CONTAINER_HOSTNAME=""
...
CONTAINER_WEB_SERVER_INT_PORT="3001"
...
CONTAINER_WEB_SERVER_VHOSTS="status.all,uptime.all"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
CONTAINER_MOUNT_DATA_MOUNT_DIR="/app/data"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set custom script - saves to /config/env/uptime.script.sh
...
CONTAINER_HOSTNAME="${CONTAINER_HOSTNAME:-${APPNAME:-uptime}}"
...
if [ -n "$UPTIME_USERNAME" ]; then
  CONTAINER_USER_NAME="$UPTIME_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${UPTIME_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
...
if [ -n "$UPTIME_PASSWORD" ]; then
  CONTAINER_USER_PASS="$UPTIME_PASSWORD"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
  if [ "$CONTAINER_USER_PASS" = "random" ]; then
    CONTAINER_USER_PASS="$(__password "${CONTAINER_PASS_LENGTH:-16}")"
  fi
  CONTAINER_USER_PASS="${UPTIME_PASSWORD:-${CONTAINER_USER_PASS:-$DEFAULT_PASSWORD}}"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
